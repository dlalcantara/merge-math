...
**Independent Test**: Start the game, click "Generate Generator" to place a generator (value 1) in the Generators Grid, then click that generator cell twice (select then activate) to produce a 1 in the Numbers Grid. Select a Numbers Grid cell, then click another non-empty cell to merge them using the active operator. Verify the values update correctly and the selection clears.
...
- What happens when the Generators Grid is full and the player clicks "Generate Generator"? → Nothing happens; no empty slot is available.
- What happens when the Numbers Grid is full and the player clicks an already-selected generator? → Nothing happens; generation requires at least one empty Numbers Grid cell.
- What happens when "Merge All Numbers" is used with only one number in the Numbers Grid? → That single value remains as the result (no change to the value).
- What happens when dividing by zero during a Cell Merge? → The result is treated as 0.
- What happens when Undo is pressed with no action history? → Nothing changes; the button is inactive or produces no effect.
...
- **FR-002**: System MUST generate and display a sorted list of 10 target integers between 1 and 1024 (inclusive) at game start, ordered from smallest to largest.
...
- **FR-006**: System MUST support Cell Selection: clicking a non-empty cell in a grid when no cell is currently selected in that grid selects it with a visual highlight.
...
- **FR-015**: System MUST support Undo: clicking Undo reverts the Numbers Grid, Generators Grid, Target List, and Action Score to the state immediately before the most recent scored action; all scored actions in the session are undoable.
...
- **Cell**: A slot in a grid; either empty or containing a single non-negative integer value.
...
- **Target**: An integer between 1 and 1024 representing a value the player must produce in the Numbers Grid and then claim.
...
- **SC-002**: All 18 functional requirements produce correct, consistent outcomes across repeated play with no incorrect values, missing updates, or phantom state.
...
- The Numbers Grid and Generators Grid are both fully empty at game start.
...
- The game is intended for modern desktop browsers; mobile and touch support are out of scope for the MVP.
