...
describe('GeneratorsSection', () => {
  const defaultProps = {
    ...
    onCellClick: vi.fn(),
    onGenerateGenerator: vi.fn(),
    onClearGenerators: vi.fn(),
  ...
  it('renders a "Generate Generator" button', () => {
    render(<GeneratorsSection {...defaultProps} />)
    expect(screen.getByRole('button', { name: /generate generator/i })).toBeInTheDocument()
  })

  it('renders a "Clear Generators Grid" button', () => {
    render(<GeneratorsSection {...defaultProps} />)
    expect(screen.getByRole('button', { name: /clear generators grid/i })).toBeInTheDocument()
  })
  ...
  it('groups grid and buttons in the same section container', () => {
    ...
    expect(section).toContainElement(screen.getByRole('button', { name: /generate generator/i }))
    expect(section).toContainElement(screen.getByRole('button', { name: /clear generators grid/i }))
  })
})
